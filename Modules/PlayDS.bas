...
dim shared as integer NotePower,NoteChan=-1

sub AddNote(NOTE as integer,DURATION as single, MODE as integer)
  
  static as integer Legato
  static as double TMRNOTE,NOTSZ,PASZ  
  dim as integer FLAG
  
  #macro SetNote(nNote)
  if NoteChan >= 0 then
    soundSetFreq(NoteChan,nNote)    
    soundSetVolume(NoteChan,127)    
    NotePower = 32767
  else
    NoteChan = soundPlayPSG(DutyCycle_50,nNote,5, 64)    
    soundSetVolume(NoteChan,127)
    NotePower = 32767
  end if
  #endmacro
...
' *******************************************************************
' *******************************************************************
sub Play(CMDS as string) 'Thread(ID as any ptr)
